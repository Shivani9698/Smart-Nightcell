/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2025 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32h7xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

extern uint8_t power_up;

extern IWDG_HandleTypeDef hiwdg1;

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

typedef enum reset_cause_e
{
    RESET_CAUSE_UNKNOWN = 0,
    RESET_CAUSE_LOW_POWER_RESET,
    RESET_CAUSE_WINDOW_WATCHDOG_RESET,
    RESET_CAUSE_INDEPENDENT_WATCHDOG_RESET,
    RESET_CAUSE_SOFTWARE_RESET,
    RESET_CAUSE_POWER_ON_POWER_DOWN_RESET,
    RESET_CAUSE_EXTERNAL_RESET_PIN_RESET,
    RESET_CAUSE_BROWNOUT_RESET,
} reset_cause_t;

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

const char * reset_cause_get_name(reset_cause_t reset_cause);
reset_cause_t reset_cause_get(void);

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define USER_SWITCH_Pin GPIO_PIN_13
#define USER_SWITCH_GPIO_Port GPIOC
#define SPI_CHIP_SELECT_Pin GPIO_PIN_4
#define SPI_CHIP_SELECT_GPIO_Port GPIOA
#define RED_LED_Pin GPIO_PIN_1
#define RED_LED_GPIO_Port GPIOB
#define BLUE_LED_Pin GPIO_PIN_2
#define BLUE_LED_GPIO_Port GPIOB
#define GREEN_LED_Pin GPIO_PIN_10
#define GREEN_LED_GPIO_Port GPIOB
#define WiFi_CHIP_ENABLE_Pin GPIO_PIN_14
#define WiFi_CHIP_ENABLE_GPIO_Port GPIOB
#define CELLULAR_PWR_KEY_Pin GPIO_PIN_15
#define CELLULAR_PWR_KEY_GPIO_Port GPIOB
#define CELLULAR_RST_Pin GPIO_PIN_15
#define CELLULAR_RST_GPIO_Port GPIOA
#define MODBUS_TX_Pin GPIO_PIN_10
#define MODBUS_TX_GPIO_Port GPIOC
#define MODBUS_RX_Pin GPIO_PIN_11
#define MODBUS_RX_GPIO_Port GPIOC
#define MODBUS_DE_Pin GPIO_PIN_12
#define MODBUS_DE_GPIO_Port GPIOC
#define MODBUS_RE_Pin GPIO_PIN_2
#define MODBUS_RE_GPIO_Port GPIOD
#define RELAY_2_Pin GPIO_PIN_3
#define RELAY_2_GPIO_Port GPIOB
#define RELAY_1_Pin GPIO_PIN_4
#define RELAY_1_GPIO_Port GPIOB
#define CHARGE_STATUS_Pin GPIO_PIN_5
#define CHARGE_STATUS_GPIO_Port GPIOB
#define PGOOD_Pin GPIO_PIN_6
#define PGOOD_GPIO_Port GPIOB
#define LORA_CHIP_RESET_Pin GPIO_PIN_9
#define LORA_CHIP_RESET_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */


void MX_TIM17_Init(void);


/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
